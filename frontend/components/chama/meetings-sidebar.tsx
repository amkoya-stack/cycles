    ...
    <div className="space-y-4 md:space-y-6">
      ...
      {ongoingMeetings.length > 0 && (
        <div className="space-y-2 md:space-y-3">
          <div className="flex items-center gap-2">
            <Radio className="h-4 w-4 md:h-5 md:w-5 text-[#f64d52] animate-pulse" />
            <h3 className="font-semibold text-sm md:text-base text-[#f64d52]">Live Now</h3>
          </div>
          <div className="space-y-2 md:space-y-3">
            {ongoingMeetings.map((meeting) => {
              ...
              return (
                <div
                  key={meeting.id}
                  className="p-2.5 md:p-3 bg-[#f64d52]/5 border border-[#f64d52]/20 rounded-lg hover:bg-[#f64d52]/10 transition-colors"
                >
                  <div className="flex items-start justify-between gap-2 mb-1.5 md:mb-2">
                    <h4 className="font-medium text-xs md:text-sm text-gray-900 flex-1">
                      {meeting.title}
                    </h4>
                    <Radio className="h-3.5 w-3.5 md:h-4 md:w-4 text-[#f64d52] animate-pulse flex-shrink-0" />
                  </div>
                  <div className="flex items-center gap-2 md:gap-3 text-xs text-gray-600 mb-2 md:mb-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="truncate max-w-[120px] md:max-w-none">{organizerName}</span>
                    </div>
                  </div>
                  {!isJoined ? (
                    <Button
                      ...
                      size="sm"
                      className="w-full bg-[#f64d52] hover:bg-[#f64d52]/90 text-white h-9 md:h-8 text-xs font-medium touch-manipulation"
                    >
                      <LogIn className="h-3.5 w-3.5 md:h-3 md:w-3 mr-1.5 md:mr-1" />
                      Join Meeting
                    </Button>
                  ) : (
                    <div className="w-full h-9 md:h-8 flex items-center justify-center text-xs font-medium text-[#f64d52] bg-[#f64d52]/10 rounded">
