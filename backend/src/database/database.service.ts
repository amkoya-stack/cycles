...
import { Injectable, OnModuleInit, OnModuleDestroy, Logger, Optional, Inject, forwardRef } from '@nestjs/common';
...
export class DatabaseService implements OnModuleInit, OnModuleDestroy {
  ...
  async query<T = any>(text: string, params?: any[]): Promise<QueryResult<T>> {
    ...
    try {
      ...
      this.logger.debug(`Executed query: ${duration}ms, ${result.rowCount} rows`);
      return result;
    } catch (error) {
      this.logger.error('Query error:', { text: text.substring(0, 200), error });
      throw error;
    }
  }
